...
(function () {
  ...
  /* Per-token flags packed by tools/lasla.py. JOIN means the token is set
     flush against the one before it: an enclitic split off as its own word
     ("populus" + "que") or a mark of punctuation. */
  var NAME = 1, PUNCT = 2, JOIN = 4;
  ...
  function displayLemma(lemma, isName, enclitic) {
    if (!lemma) return "";
    if (enclitic) return "-" + lemma;
  ...
  function render(book) {
    ...
    if (work.note) {
      html.push('<div class="prose-note' + (work.auto ? " auto-note" : "") +
        '">' + esc(work.note) + "</div>");
    }
    ...
    for (var i = 0; i < book.lines.length; i++) {
      ...
      for (var j = 0; j < line.w.length; j++) {
        var w = line.w[j];
        var flags = w[3];
        var space = pieces.length && !(flags & JOIN) ? " " : "";

        // Punctuation carries no analysis worth a popup, so it is set as
        // plain text rather than made clickable.
        if (flags & PUNCT) {
          pieces.push(space + '<span class="punct">' + esc(w[0]) + "</span>");
          continue;
        }
  ...
  function showPopup(span) {
    ...
    if (t.lemma) {
      el.logeion.href = "https://logeion.uchicago.edu/" +
        encodeURIComponent(displayLemma(t.lemma, t.name, false));
  ...
  function buildWorkList() {
    ...
    for (var i = 0; i < works.length; i++) {
      ...
      html += '<li><button type="button" role="radio" aria-checked="false" data-work="' +
        ...
        '<span class="div-title">' + esc(w.title) +
        (w.auto ? ' <span class="auto-tag" title="Morphology generated by a tagger, not hand-checked">auto</span>' : "") +
        "</span></button></li>";
    }
